# The method: working with agents (proposal, second draft)

A proposal to replace the shipped `method.md`, drafted 2026-09-20 from
the two curation readings, the first day's log, and a rethink the
person asked for that evening: the agents we start are colleagues we
trust, not helpers we manage. The companion, `framing.md`, is how to
write the feature and the brief. Nothing here is applied until a
person says so.
