...
`GET /api/projects/:id/commits` lists the project's commits across its
...
request). Who made each commit is attributed in three steps.

First, the turn that made it. When a vendor reports a commit (Claude's
`vcs_state_changed`; Codex and Copilot report none, so only Claude's
commits get this), the manager resolves HEAD in the commit's cwd at that
live moment and records the row in `commit_attributions` (project, repo,
hash → agent, session, the transcript item index of the "committed" line,
time). It records only for a live event, not a replay: on a rebuild
`git rev-parse HEAD` would give the current HEAD, not the one at the
commit's moment, and the row is already on disk — item indexes are stable
per session, so it survives the rebuild. A commit whose cwd is not one of
the project's repositories, or whose HEAD will not resolve (a race with a
push), is logged and dropped. This is what lets the changes tab name the
agent and, later, link to the turn.

Second, when there is no such record (a commit from before this existed,
or an agent the manager did not see commit), the run window: the commits
in a run's `base_commit..end_commit` (in that run's own repository; an
open run runs to HEAD) take that run's agent, and when a commit falls in
more than one window — two features' rounds interleaved in one repository
— the innermost wins, the run whose base is the latest ancestor of the
commit, ties broken by the most recent start. The feature slug always
comes from the run window (the turn record does not carry it), so a
commit can name its agent from the record and its feature from the
window at once.

Third, a commit in neither carries the git author — the fallback for
commits made outside the manager. The per-commit diff (`GET
.../commits/:repo/:hash`) is the commit against its first parent, read
the same way. Still not attributed to an agent: a commit an agent makes
in a sibling repository (its cwd is one repo, and Claude's event names
that cwd), if that commit is neither the recorded HEAD nor in a run
window.
...
GET    /api/projects/:id/commits?repo=&feature=&agent=&since=&limit=&count= -> { commits: [{ repo, hash, shortHash, subject, author, at, agent, agentId, feature, sessionId, item, unread }], working: [{ repo, agent, files }], sinceCount }; the project's commits across its repositories, newest first; `agent`/`agentId` from the recorded turn that made it, else a run window, else null (the git author is then who); `feature` from the run window; `sessionId`/`item` (the transcript item index) when a turn made it and it was recorded; `unread` is after the caller's read cursor; `working` is the repos with uncommitted work and the agent of a run still open there; `sinceCount` is the unread commit count; `count=1` returns only that count; nothing is cached, so a rebase or amend shows at once
