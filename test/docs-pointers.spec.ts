...
/**
 * The method moved from `docs/method.md` to the repository root, where
 * the installer can ship it like the harness note and the models file.
 * Every pointer here has to follow, and a stale one is silent: the text
 * still reads, it just names a file nobody has.
 */
...
describe('documentation pointers', () => {
  it('nothing in the repository points at the old path of the method', () => {
    ...
    try {
      hits = execFileSync(
        'git',
        ['grep', '-l', '--', 'docs/method.md', ':!features/'],
